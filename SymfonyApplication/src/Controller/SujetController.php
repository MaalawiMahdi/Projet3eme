...
class SujetController extends AbstractController
{
     ...
     */
    public function afficher($idboard,SessionInterface $session): Response
    {
        if(is_null($session->get('user'))||$session->get('user')->getType()!="societe"){
        return $this->redirectToRoute('user_inscription');
    }else if($session->get('user')->getSociete()->getBoard()->getId()!=$idboard){
        return $this->redirectToRoute('AfficherSujet',['idboard'=>$session->get('user')->getSociete()->getBoard()->getId()]);
    }
     ...
     */
    public function ImprimerS($idboard,SessionInterface $session): Response
     ...
     */
    public function AjouterSujet(Request $request,$idboard,SessionInterface $session)
    {    if(is_null($session->get('user'))||$session->get('user')->getType()!="societe"){
        return $this->redirectToRoute('user_inscription');
    }else if($session->get('user')->getSociete()->getBoard()->getId()!=$idboard){
        return $this->redirectToRoute('AjouterSujet',['idboard'=>$session->get('user')->getSociete()->getBoard()->getId()]);
    }
     ...
     */
    public function SupprimerSujet($idboard,$id,SessionInterface $session)
    {  if(is_null($session->get('user'))||$session->get('user')->getType()!="societe"){
        return $this->redirectToRoute('user_inscription');
    }else if($session->get('user')->getSociete()->getBoard()->getId()!=$idboard){
        return $this->redirectToRoute('SupprimerSujet',['idboard'=>$session->get('user')->getSociete()->getBoard()->getId()]);
    }
     ...
     */
    public function ModifiererSujet(Request $request,$idboard,$id,SessionInterface $session)
    { if(is_null($session->get('user'))||$session->get('user')->getType()!="societe"){
        return $this->redirectToRoute('user_inscription');
    }else if($session->get('user')->getSociete()->getBoard()->getId()!=$idboard){
        return $this->redirectToRoute('ModifierSujet',['idboard'=>$session->get('user')->getSociete()->getBoard()->getId()]);
    }
        $sujet = $this->getDoctrine()->getRepository( Sujet::class)->find($id);
        $form = $this->createForm(SujetType::class,$sujet);
   ...
   /**
     ...
     */
    public function ConsulterSujet(Request $request,$id,$idboard,SessionInterface $session)
    {   if(is_null($session->get('user'))){
        return $this->redirectToRoute('user_inscription');
    }else if($session->get('user')->getSociete()->getBoard()->getId()!=$idboard){
        return $this->redirectToRoute('ConsulterSujet',['idboard'=>$session->get('user')->getSociete()->getBoard()->getId()]);
    }
     ...
     */
    public function afficherFront($idboard,Request $request, PaginatorInterface $paginator,SessionInterface $session): Response
    {
        ...
        return $this->render('sujet/frontaffichersujet.html.twig',['list'=>$list, 'board_id'=>$idboard, 'path'=>$session->get('path'),'texte'=>$session->get('texte'),]);
    }
    /**
    ...
    public function ConsulterSujetFront(Request $request,$idboard,$id,SessionInterface $session)
    {
        ...
        return $this->render('sujet/consultersujetfront.html.twig',['e'=>$e,'sujet'=>$sujet, 'board_id'=>$idboard, 'f'=>$f->createView(),'listc'=>$listc, 'user'=>$user ,'path'=>$session->get('path'),'texte'=>$session->get('texte'),]);
    }

    /**
     * @Route("/AjouterNote/{idsujet}/{iduser}/{v}", name="AjouterNoteSujet")
     */
    public function AjouterNoteSujet($idsujet,$iduser,$v,SessionInterface $session)
    { if(is_null($session->get('user'))||$session->get('user')->getType()=="admin"){
        return $this->redirectToRoute('user_inscription');
     ...
     */
    public function Stats($idboard,SessionInterface $session)
    { if(is_null($session->get('user'))||$session->get('user')->getType()=="admin"){
        return $this->redirectToRoute('user_inscription');
    }
    else if($session->get('user')->getSociete()->getBoard()->getId()!=$idboard){
        return $this->redirectToRoute('Stats',['idboard'=>$session->get('user')->getSociete()->getBoard()->getId()]);
    }
        ...
        $ob->series($series);
        return $this->render('sujet/statistics.html.twig', ['chart' => $ob , 'path'=>$session->get('path'),'texte'=>$session->get('texte'),]);
    }
}